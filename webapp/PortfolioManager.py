...
class PortfolioManager:
    ...
    def get_price(self, ticker, transaction_date=None):
        ...
        If transaction_date is provided (format: "YYYY-MM-DD"), return the closing price on that day,
        or, if missing, the nearest previous trading day's price.
        Otherwise, return the latest available market price.
        """
        try:
            ...
            max_lookback = 10  # Maximum number of days to look back for data.
            days_back = 1
            price = None
            used_date = None
    ...
    def check_portfolio(self, portfolio_id):
        ...
        if positions:
            ...
            for ticker, data in positions.items():
                ...
                print(f"{pos_type}: {data['name']} ({ticker}) - Avg Price: ${avg_price:.2f}, Market Price: ${market_price:.2f}, Quantity: {quantity}, Current Value: ${current_val:.2f}, P&L: ${pnl:.2f}")
            ...
            self.portfolio_performance(portfolio_id, total_long_val, total_short_val, total_long_pnl, total_short_pnl)
        else:
    ...
    def portfolio_performance(self, portfolio_id, total_long_val, total_short_val, total_long_pnl, total_short_pnl):
        try:
            self.db.get_cursor().execute("SELECT MIN(transaction_date) FROM transactions WHERE portfolio_id = ?", (portfolio_id,))
            earliest_date_str = self.db.get_cursor().fetchone()[0]
            ...
            if total_long_val > 0:
                ...
                annualized_long_return = self.compute_annualized_return(initial_long_investment, total_long_val, years)
            ...
            if total_short_val < 0:
                ...
                annualized_short_return = self.compute_annualized_return(initial_short_investment, abs(total_short_val), years)
                if annualized_short_return is not None:
                    print(f"Annualized Short Portfolio Return (%): {annualized_short_return * 100:.2f}%")
            else:
            ...
            annualized_total_return = self.compute_annualized_return(initial_total_investment, abs(total_val), years)
        ...
        if transactions:
            try:
                with open(filename, "w", newline="") as csvfile:
                    ...
                    writer.writerow(["ticker", "name", "transaction_date", "order_type", "price", "quantity", "limit_price"])
    ...
    def visualise_portfolio(self, portfolio_id):
        ...
        self.db.get_cursor().execute("SELECT transaction_date, ticker, price, quantity FROM transactions WHERE portfolio_id = ? ORDER BY transaction_date, id", (portfolio_id,))
        transactions = self.db.get_cursor().fetchall()
        if not transactions:
